...
[libraries]
mapvina-composeMaterial3 = { module = "io.github.mapvina.compose:mapvina-compose-material3", version = "{{ gradle.release_version }}" }
```

```kotlin title="build.gradle.kts"
commonMain.dependencies {
  implementation(libs.mapvina.composeMaterial3)
}
```

Then, disable the default ornaments and add the Material controls after the
`MapvinaMap` in a shared `Box`.

```kotlin title="App.kt"
-8<- "demo-app/src/commonMain/kotlin/io/github/mapvina/compose/docsnippets/Material3.kt:controls"
```
...
```kotlin title="App.kt"
-8<- "demo-app/src/commonMain/kotlin/io/github/mapvina/compose/docsnippets/Material3.kt:disappearing-controls"
```
