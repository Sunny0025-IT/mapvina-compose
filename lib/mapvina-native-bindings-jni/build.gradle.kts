...
val DesktopVariant.sharedLibraryName: String
  get() =
    when (os) {
      "windows" -> "mapvina-jni.${sharedLibraryExtension}"
      else -> "libmapvina-jni.${sharedLibraryExtension}"
    }
...
mavenPublishing {
  pom {
    name = "MapVina Native Bindings (Natives)"
    description = "JNI native libraries for MapVina Native Bindings."
...
if (config.shouldConfigureForPublishing) {
  ...
  tasks.register<Exec>("configureCMake") {
    ...
    dependsOn(":lib:mapvina-native-bindings:kspKotlinDesktop")
    ...
    inputs.file(layout.projectDirectory.file("vendor/mapvina-native/CMakeLists.txt"))
