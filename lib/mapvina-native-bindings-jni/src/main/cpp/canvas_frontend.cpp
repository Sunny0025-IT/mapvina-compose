...
CanvasRenderer_class::render(JNIEnv* env, jCanvasRenderer renderer) {
  try {
    ...
    auto* frontend = reinterpret_cast<mapvina_jni::CanvasRenderer*>(
...
void JNICALL CanvasRenderer_class::setSize(
  ...
  try {
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    auto* renderer = reinterpret_cast<mapvina_jni::CanvasRenderer*>(
      java_classes::get<CanvasRenderer_class>().getNativePointer(
        env, canvasFrontend
      )
    );
    renderer->setSize(mapvina_jni::convertSize(env, size));
  } catch (const std::exception& e) {
    smjni::java_exception::translate(env, e);
  }
}
...
auto JNICALL CanvasRenderer_class::alloc(
  ...
  try {
    ...
    return reinterpret_cast<jlong>(
      new mapvina_jni::CanvasRenderer(env, renderer, pixelRatio)
    );
  } catch (const std::exception& e) {
    smjni::java_exception::translate(env, e);
    return 0;
...
CanvasRenderer_class::destroy(JNIEnv* env, jclass /*unused*/, jlong ptr) {
  try {
    ...
    delete reinterpret_cast<mapvina_jni::CanvasRenderer*>(ptr);
  } catch (const std::exception& e) {
    smjni::java_exception::translate(env, e);
  }
}
