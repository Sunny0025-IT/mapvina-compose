...
import static com.mapvina.android.location.LocationComponentConstants.ACCURACY_RADIUS_ANIMATION_DURATION;
import static com.mapvina.android.location.LocationComponentConstants.COMPASS_UPDATE_RATE_MS;
import static com.mapvina.android.location.LocationComponentConstants.MAX_ANIMATION_DURATION_MS;
import static com.mapvina.android.location.LocationComponentConstants.TRANSITION_ANIMATION_DURATION_MS;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_CAMERA_COMPASS_BEARING;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_CAMERA_GPS_BEARING;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_CAMERA_LATLNG;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_LAYER_ACCURACY;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_LAYER_COMPASS_BEARING;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_LAYER_GPS_BEARING;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_LAYER_LATLNG;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_PADDING;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_PULSING_CIRCLE;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_TILT;
import static com.mapvina.android.location.MapVinaAnimator.ANIMATOR_ZOOM;
import static com.mapvina.android.location.Utils.immediateAnimation;
import static com.mapvina.android.location.Utils.normalize;
import static com.mapvina.android.location.Utils.shortestRotation;
...
final class LocationAnimatorCoordinator {
  ...
  @VisibleForTesting
  final SparseArray<MapVinaAnimator> animatorArray = new SparseArray<>();
  ...
  private final MapVinaAnimatorProvider animatorProvider;
  private final MapVinaAnimatorSetProvider animatorSetProvider;
  ...
  @VisibleForTesting
  final SparseArray<MapVinaAnimator.AnimationsValueChangeListener> listeners = new SparseArray<>();

  LocationAnimatorCoordinator(@NonNull Projection projection, @NonNull MapVinaAnimatorSetProvider animatorSetProvider,
                              @NonNull MapVinaAnimatorProvider animatorProvider) {
  ...
  void updateAnimatorListenerHolders(@NonNull Set<AnimatorListenerHolder> listenerHolders) {
    ...
    for (int i = 0; i < animatorArray.size(); i++) {
      @MapVinaAnimator.Type int animatorType = animatorArray.keyAt(i);
      if (listeners.get(animatorType) == null) {
        MapVinaAnimator animator = animatorArray.get(animatorType);
        if (animator != null) {
  ...
  /**
   * Initializes the {@link PulsingLocationCircleAnimator}, which is a type of {@link MapVinaAnimator}.
  ...
  void startLocationComponentCirclePulsing(LocationComponentOptions options) {
    ...
    MapVinaAnimator.AnimationsValueChangeListener listener = listeners.get(ANIMATOR_PULSING_CIRCLE);
    if (listener != null) {
  ...
  void feedNewZoomLevel(double targetZoomLevel, @NonNull CameraPosition currentCameraPosition, long animationDuration,
                        @Nullable MapVinaMap.CancelableCallback callback) {
  ...
  private LatLng getPreviousLayerLatLng() {
    ...
    MapVinaAnimator latLngAnimator = animatorArray.get(ANIMATOR_LAYER_LATLNG);
  ...
  private float getPreviousLayerGpsBearing() {
    MapVinaFloatAnimator animator = (MapVinaFloatAnimator) animatorArray.get(ANIMATOR_LAYER_GPS_BEARING);
  ...
  private float getPreviousLayerCompassBearing() {
    MapVinaFloatAnimator animator = (MapVinaFloatAnimator) animatorArray.get(ANIMATOR_LAYER_COMPASS_BEARING);
  ...
  private float getPreviousAccuracyRadius() {
    MapVinaAnimator animator = animatorArray.get(ANIMATOR_LAYER_ACCURACY);
  ...
  private void createNewLatLngAnimator(@MapVinaAnimator.Type int animatorType,
                                       @NonNull LatLng previous, @NonNull LatLng target) {
    createNewLatLngAnimator(animatorType, new LatLng[] {previous, target});
  }

  private void createNewLatLngAnimator(@MapVinaAnimator.Type int animatorType, LatLng[] values) {
    cancelAnimator(animatorType);
    MapVinaAnimator.AnimationsValueChangeListener listener = listeners.get(animatorType);
    if (listener != null) {
      animatorArray.put(animatorType, animatorProvider.latLngAnimator(values, listener, maxAnimationFps));
    }
  }

  private void createNewFloatAnimator(@MapVinaAnimator.Type int animatorType, float previous, float target) {
    createNewFloatAnimator(animatorType, new Float[] {previous, target});
  }

  private void createNewFloatAnimator(@MapVinaAnimator.Type int animatorType, @NonNull @Size(min = 2) Float[] values) {
    cancelAnimator(animatorType);
    MapVinaAnimator.AnimationsValueChangeListener listener = listeners.get(animatorType);
    if (listener != null) {
  ...
  private void createNewCameraAdapterAnimator(@MapVinaAnimator.Type int animatorType,
  ...
  private void createNewPaddingAnimator(@MapVinaAnimator.Type int animatorType,
  ...
  private void playAnimators(long duration, @MapVinaAnimator.Type int... animatorTypes) {
    List<Animator> animators = new ArrayList<>();
    for (@MapVinaAnimator.Type int animatorType : animatorTypes) {
   ...
   * customizable animation parameters than the other {@link MapVinaAnimator}s.
   */
  ...
  private boolean resetCameraLatLngAnimation(@NonNull CameraPosition currentCameraPosition) {
    MapVinaLatLngAnimator animator = (MapVinaLatLngAnimator) animatorArray.get(ANIMATOR_CAMERA_LATLNG);
    if (animator == null) {
      return false;
    }
  ...
  private void resetCameraGpsBearingAnimation(@NonNull CameraPosition currentCameraPosition, boolean isGpsNorth) {
    MapVinaFloatAnimator animator = (MapVinaFloatAnimator) animatorArray.get(ANIMATOR_CAMERA_GPS_BEARING);
    if (animator == null) {
      return;
    }
  ...
  private void resetCameraCompassAnimation(@NonNull CameraPosition currentCameraPosition) {
    MapVinaFloatAnimator animator =
      (MapVinaFloatAnimator) animatorArray.get(ANIMATOR_CAMERA_COMPASS_BEARING);
    if (animator == null) {
      return;
    }
  ...
  void resetAllLayerAnimations() {
    MapVinaLatLngAnimator latLngAnimator = (MapVinaLatLngAnimator) animatorArray.get(ANIMATOR_LAYER_LATLNG);
    MapVinaFloatAnimator gpsBearingAnimator = (MapVinaFloatAnimator) animatorArray.get(ANIMATOR_LAYER_GPS_BEARING);
    MapVinaFloatAnimator compassBearingAnimator =
      (MapVinaFloatAnimator) animatorArray.get(ANIMATOR_LAYER_COMPASS_BEARING);
    MapVinaFloatAnimator accuracyAnimator =
      (MapVinaFloatAnimator) animatorArray.get(ANIMATOR_LAYER_ACCURACY);
  ...
  private void cancelAnimator(@MapVinaAnimator.Type int animatorType) {
    MapVinaAnimator animator = animatorArray.get(animatorType);
    if (animator != null) {
      animator.cancel();
