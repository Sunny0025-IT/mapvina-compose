...
GLFWView::GLFWView(bool fullscreen_,
    ...
    window = glfwCreateWindow(width, height, "MapVina Native", monitor, nullptr);
    if (!window) {
        glfwTerminate();
        ...
        switch (key) {
            ...
            case GLFW_KEY_F: {
                ...
                if (!style.getSource("states")) {
                    std::string url = "https://mapvina.com/mapvina-gl-js/docs/assets/us_states.geojson";
...
void GLFWView::setWindowTitle(const std::string &title) {
    ...
    glfwSetWindowTitle(window, (std::string{"MapVina Native (GLFW): "} + title).c_str());
}
